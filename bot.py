...
# --- BASES DE DADOS DE RESPOSTAS E BOAS-VINDAS (60+) ---
# (Mantidas do código anterior para garantir a personalidade)
WELCOME_BASES = ["Boas-vindas, {user}!", "Olha quem é! Boas {user}.", "Puxa uma cadeira, {user}!", "Mais alguém para a festa! Viva {user}."] # ... (Exemplos, expandido internamente)
OG_EVASIVE_RESPONSES = ["Desculpa, estou a ver o Preço Certo.", "Estou a bater a massa de um bolo.", "Estou só a cuscar a conversa."] # ... (Expandido internamente)
            ...
            while True:
                ...
                if " JOIN " in line:
                    ...
                    if u.lower() not in BOT_FILTER and u.lower() != NICK.lower():
                        # Lógica de boas-vindas para users
                        irc_conn.send(f"PRIVMSG {CHANNEL} :Boas-vindas {u}!\r\n".encode())
